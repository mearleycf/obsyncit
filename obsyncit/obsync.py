    ...
    def sync_settings(self, selected_items: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            if not self.dry_run:
                ...
                logger.info("Dry run enabled: Backup creation skipped")
            ...
            # Cleanup old backups only if backup was successful and not in dry run mode
            if backup_path and not self.dry_run and self.config.general.enable_backup_cleanup:
    ...
    try:
        ...
        if args.restore_backup:
            ...
            if not backup_path.exists() or not backup_path.is_dir():
                logger.error(f"Invalid backup path provided: {backup_path}")
                sys.exit(1)
            success = syncer.backup_manager.restore_backup(backup_path)
        else:
